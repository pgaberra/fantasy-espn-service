...
public class InternalApiKeyFilter extends OncePerRequestFilter {
    ...
    // Fail closed: this service stores per-user ESPN session cookies, so it must never serve
    // an unauthenticated internal API. If no key is configured, refuse to start rather than
    // silently allowing all callers. Run any environment (local included) with INTERNAL_API_KEY
    // set; tests provide one.
    @PostConstruct
    void requireApiKey() {
    ...
    // actuator health/info probes only, so a traversal such as
    // /actuator/health/../../api/v1/espn/leagues cannot slip a protected path past the filter
    // under an exempt prefix. Unlike yahoo-service, ESPN has no browser OAuth callback, so there
    // is no public endpoint to exempt beyond the health/info probes.
